...
export async function POST(req: Request) {
  ...
  try {
    ...
    const sites = await prisma.site.findMany({
      // Billing disabled: process any site the user enabled auto-index on
      // (per-site toggle), regardless of plan.
      where: {
    ...
    for (const site of sites) {
      ...
      try {
        ...
        // Billing disabled: everyone gets daily reports (gated only by emailReports).
        const isReportDay = true;
