...
class ReportGenerator:
    ...
    async def generate(self, audit: AuditResult, brand: dict | None = None, theory_level: str = "compact") -> str:
        ...
        # Render template + write file off the event loop (Jinja rendering and
        # disk I/O are blocking; keep the loop free for SSE and other audits).
    ...
    async def generate_pdf(self, audit: AuditResult, brand: dict | None = None, show_watermark: bool = True, theory_level: str = "compact") -> str:
        ...
        if brand and brand.get("logo_url"):
            logo_bytes = await asyncio.to_thread(self._fetch_logo_bytes, brand["logo_url"])
            if logo_bytes:
        ...
        # WeasyPrint is CPU-bound; run off the event loop so PDF rendering
        # doesn't freeze SSE streams and other concurrent audits.
    ...
    async def generate_docx(self, audit: AuditResult, brand: dict | None = None, show_watermark: bool = True, theory_level: str = "compact") -> str:
        ...
        # =============================================
        if brand_logo_url:
            logo_bytes = await asyncio.to_thread(self._fetch_logo_bytes, brand_logo_url)
            if logo_bytes:
                try:
        ...
        # Save document (python-docx serialization is blocking; offload it)
