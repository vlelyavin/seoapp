...
async def cleanup_old_audits():
    ...
    while True:
        ...
        now = time.time()
        terminal = (AuditStatus.COMPLETED, AuditStatus.FAILED)
        # Only evict finished audits, so a long-running one isn't deleted mid-flight.
        # A hard cap (4x TTL) still reclaims anything wedged in a non-terminal state.
        to_remove = [
            aid for aid, (audit, ts) in audits.items()
            if now - ts > AUDIT_TTL
            and (audit.status in terminal or now - ts > AUDIT_TTL * 4)
        ]
        for aid in to_remove:
            audits.pop(aid, None)
            broadcast_channels.pop(aid, None)
            audit_progress_history.pop(aid, None)
...
async def _run_audit_inner(audit_id: str, request: AuditRequest, state: Dict):
    ...
    try:
        ...
        if not speed_only:
            ...
            # Running aggregates, updated once per page, so each progress event is
            # O(links on this page) instead of O(whole corpus) rescans (was O(n^2)).
            agg_internal_links: Set[str] = set()
            agg = {"seen": 0, "external": 0, "e4xx": 0, "e5xx": 0, "r3xx": 0,
                   "lt_sum": 0.0, "lt_count": 0}

            async def progress_callback(page: PageData):
                agg["seen"] += 1
                agg_internal_links.update(page.internal_links)
                agg["external"] += len(page.external_links)
                if 400 <= page.status_code < 500:
                    ...
                    agg["r3xx"] += 1
                if page.load_time > 0:
                    agg["lt_sum"] += page.load_time
                    agg["lt_count"] += 1

                seen = agg["seen"]
                progress = min(seen / max_pages * 40, 40)
                avg_rt = (
                    round(agg["lt_sum"] / agg["lt_count"] * 1000, 0)
                    if agg["lt_count"] else None
                )
                elapsed = time.time() - crawl_started_ts
                est_seconds = None
                if seen > 1 and elapsed > 0:
                    rate = seen / elapsed
                    remaining = max_pages - seen
                    if remaining > 0:
                        est_seconds = int(remaining / rate)
                await emit_progress(ProgressEvent(
                    status=AuditStatus.CRAWLING,
                    progress=progress,
                    message=t("progress.crawling_pages", progress_lang, count=seen),
                    ...
                    analyzers_total=analyzers_total,
                    analyzers_completed=completed_total(),
                    links_found=len(agg_internal_links),
                    external_links_count=agg["external"],
                    errors_4xx=agg["e4xx"],
                    errors_5xx=agg["e5xx"],
                    redirects_3xx=agg["r3xx"],
            ...
            # Bounded by a timeout so a wedged browser can't hold the audit slot.
            try:
                from .screenshots import screenshot_capture
                audit.homepage_screenshot = await asyncio.wait_for(
        ...
        # Bounded so a wedged renderer fails the audit instead of holding the slot.
        report_path = await asyncio.wait_for(
            generator.generate(audit), timeout=settings.REPORT_TIMEOUT
        )
        logger.info(
